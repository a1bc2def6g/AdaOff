...
class Params:
    ...
    def loadHFTransformerJson(model: 'LazyModel', config_path: 'Path') -> 'Params':
        ...
        n_ff    = config["ffn_dim"];
        n_kv_head = config.get("num_attention_heads")
...
class SentencePieceVocab:
    def __init__(self, fname_tokenizer: Path, fname_added_tokens: Optional[Path], vocabtype: Optional[str]) -> None:
        ...
        if fname_added_tokens is not None:
            ...
            added_tokens = {'<mask1>':50265,'<mask2>':50266,'<mask3>':50267,'<mask4>':50268,'<mask5>':50269,'<mask6>':50270,'<mask7>':50271,}
        if self.vocabtype == "bpe":
...
def merge_multifile_models(models_plus: List[ModelPlus]) -> ModelPlus:
    ...
    if any("decoder.embed_tokens.weight" in mp.model for mp in models_plus):
...
def convert_transformers_to_orig(model: LazyModel, params: Params) -> LazyModel:
    out: LazyModel = {}
    if("model.decoder.embed_tokens.weight" in model.keys()):
        out["tok_embeddings.weight"] = model["model.decoder.embed_tokens.weight"]
        out["tok_embeddings_pos.weight"] = model["model.decoder.embed_positions.weight"]
        out["norm.weight"] = model["model.decoder.final_layer_norm.weight"]
        # out["norm.bias"] = model["model.decoder.final_layer_norm.bias"]
        if("lm_head.weight" in model.keys()):
            out["output.weight"] = model["lm_head.weight"]
        else:
            out["output.weight"] = model["model.decoder.embed_tokens.weight"]

        for i in itertools.count():
            if f"model.decoder.layers.{i}.self_attn.q_proj.weight" in model:
                out[f"layers.{i}.attention.wq.weight"] = permute_lazy(model[f"model.decoder.layers.{i}.self_attn.q_proj.weight"], params.n_head)
                out[f"layers.{i}.attention.wk.weight"] = permute_lazy(model[f"model.decoder.layers.{i}.self_attn.k_proj.weight"], params.n_head, params.n_kv_head)
                out[f"layers.{i}.attention.wv.weight"] = model[f"model.decoder.layers.{i}.self_attn.v_proj.weight"]
            elif f"model.decoder.layers.{i}.self_attn.W_pack.weight" in model:
            ...
            out[f"layers.{i}.attention.wo.weight"] = model[f"model.decoder.layers.{i}.self_attn.out_proj.weight"]

            out[f"layers.{i}.feed_forward.w1.weight"] = model[f"model.decoder.layers.{i}.fc1.weight"]
            out[f"layers.{i}.feed_forward.w2.weight"] = model[f"model.decoder.layers.{i}.fc2.weight"]

            out[f"layers.{i}.attention_norm.weight"] = model[f"model.decoder.layers.{i}.self_attn_layer_norm.weight"]
            out[f"layers.{i}.ffn_norm.weight"] = model[f"model.decoder.layers.{i}.final_layer_norm.weight"]
    else:
        out["tok_embeddings.weight"] = model["decoder.embed_tokens.weight"]
        out["tok_embeddings_pos.weight"] = model["decoder.embed_positions.weight"]
        out["norm.weight"] = model["decoder.final_layer_norm.weight"]
        # out["norm.bias"] = model["decoder.final_layer_norm.bias"]
        if("lm_head.weight" in model.keys()):
            out["output.weight"] = model["lm_head.weight"]
        else:
            out["output.weight"] = model["decoder.embed_tokens.weight"]

        for i in itertools.count():
            if f"decoder.layers.{i}.self_attn.q_proj.weight" in model:
                out[f"layers.{i}.attention.wq.weight"] = permute_lazy(model[f"decoder.layers.{i}.self_attn.q_proj.weight"], params.n_head)
                out[f"layers.{i}.attention.wk.weight"] = permute_lazy(model[f"decoder.layers.{i}.self_attn.k_proj.weight"], params.n_head, params.n_kv_head)
                out[f"layers.{i}.attention.wv.weight"] = model[f"decoder.layers.{i}.self_attn.v_proj.weight"]
            elif f"model.decoder.layers.{i}.self_attn.W_pack.weight" in model:
                out[f"layers.{i}.attention.wq.weight"] = permute_part_lazy(model[f"model.layers.{i}.self_attn.W_pack.weight"], 0, params.n_head)
                out[f"layers.{i}.attention.wk.weight"] = permute_part_lazy(model[f"model.layers.{i}.self_attn.W_pack.weight"], 1, params.n_head)
                out[f"layers.{i}.attention.wv.weight"] = part_lazy(model[f"model.layers.{i}.self_attn.W_pack.weight"], 2)
            ...
            out[f"layers.{i}.attention.wo.weight"] = model[f"decoder.layers.{i}.self_attn.out_proj.weight"]

            out[f"layers.{i}.feed_forward.w1.weight"] = model[f"decoder.layers.{i}.fc1.weight"]
            out[f"layers.{i}.feed_forward.w2.weight"] = model[f"decoder.layers.{i}.fc2.weight"]

            out[f"layers.{i}.attention_norm.weight"] = model[f"decoder.layers.{i}.self_attn_layer_norm.weight"]
            out[f"layers.{i}.ffn_norm.weight"] = model[f"decoder.layers.{i}.final_layer_norm.weight"]
    return out
...
def main(args_in: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument("--vocabtype", default='bpe', choices=["spm", "bpe"], help="vocab format (default: spm)")
