...
static   cudaStream_t transfer_k_stream;
static   cudaStream_t transfer_v_stream;
static   cudaStream_t transfer_w_stream;
...
static __global__ void mul_mat_vec_nc_f16_f32( // nc == non-contiguous
    ...
    const half * x = (const half *) vx; //V cache   
...
//     transfer_matrix_kernel<cpy_1_f16_f16><<<num_blocks, threads_per_block>>>(src_ptr, dst_ptr, n_embd_gqa, N+n_past, total_col);
...
//     if(cpu2gpu){
//         kind = cudaMemcpyHostToDevice;
//         dst_ptr = (char *) extra->data_device[g_main_device]+(tensor->nb[0]*n_embd_gqa)*il*n_ctx;
//         src_ptr = (char *) tensor->data+(tensor->nb[0]*n_embd_gqa)*il*n_ctx;
//     }
//     else{
//         kind = cudaMemcpyDeviceToHost;
//         src_ptr = (char *) extra->data_device[g_main_device]+(tensor->nb[0]*n_embd_gqa)*il*n_ctx;
//         dst_ptr = (char *) tensor->data+(tensor->nb[0]*n_embd_gqa)*il*n_ctx;
//     }
...
//     if(cpu2gpu==1){
//         struct ggml_tensor_extra_gpu * extra = (ggml_tensor_extra_gpu * ) dst->extra;
//         kind = cudaMemcpyHostToDevice;
//         dst_ptr = (void *) extra->data_device[g_main_device]+(2*n_embd_gqa)*il*n_ctx;
//         src_ptr = (void *) dst->data+(2*n_embd_gqa)*il*n_ctx;
//     }
//     else{
//         struct ggml_tensor_extra_gpu * extra = (ggml_tensor_extra_gpu * ) dst->extra;
//         kind = cudaMemcpyDeviceToHost;
//         src_ptr = (void *) extra->data_device[g_main_device]+(2*n_embd_gqa)*il*n_ctx;
...
//     dst_ptr = (void *) extra->data_device[g_main_device];
//     src_ptr = (void *) dst->data;

//     CUDA_CHECK(cudaMemcpyAsync(dst_ptr, src_ptr, count, kind, transfer_w_stream));
//     CUDA_CHECK(cudaDeviceSynchronize());
//     // std::cout << "exe offload: " << n_past << std::endl;
// }
...
//     struct ggml_tensor_extra_gpu * extra_v = (ggml_tensor_extra_gpu * ) v_mat->extra;
//     struct ggml_tensor_extra_gpu * extra_s = (ggml_tensor_extra_gpu * ) s_mat->extra;
//     struct ggml_tensor_extra_gpu * extra_dst = (ggml_tensor_extra_gpu * ) dst->extra;

//     half * v_mat_ptr = (half *) extra_v->data_device[g_main_device];

//     float * s_mat_ptr = (float *) extra_s->data_device[g_main_device];

//     // float * dst_tmp_ptr;
//     // cudaMalloc(&dst_tmp_ptr, tgt_token_num*n_embd_gqa*sizeof(float));
//     float * dst_ptr = (float *) extra_dst->data_device[g_main_device];
//     // extra_dst->data_device[g_main_device] = dst_tmp_ptr;
...
//     struct ggml_tensor_extra_gpu * extra_src0 = (ggml_tensor_extra_gpu * ) src0->extra;
//     struct ggml_tensor_extra_gpu * extra_dst = (ggml_tensor_extra_gpu * ) dst->extra;

//     float * src0_dd = (float *) extra_src0->data_device[g_main_device];
//     float * dst_dd = (float *) extra_dst->data_device[g_main_device];
...
inline void ggml_cuda_op_mul_mat_cublas(
    ...
    // std::cout << "src[0]->name: " << src0->name << std::endl;
    // std::cout << "src[1]->name: " << src1->name << std::endl;
...
static void ggml_cuda_op(const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst,
    ...
    // std::cout << "src[1]->name: " << src1->name << std::endl;
    // std::cout << "dst->name: " << dst->name << std::endl;
    // std::cout << "op->name: " << dst->op << std::endl;
    if (dst->backend == GGML_BACKEND_CPU) {
        CUDA_CHECK(cudaSetDevice(g_main_device));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
...
bool ggml_cuda_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor){
    ...
    // if(strcmp(tensor->name,"KQ")==0){
    //     std::cout << "tensor->name: " << tensor->name << std::endl;
    //     std::cout << "any_on_device " << any_on_device << std::endl;
    // }
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL_MAT:
            ...
            break;
        case GGML_OP_MAT_MUL_CONT:
            if (!any_on_device && !ggml_cuda_can_mul_mat(tensor->src[0], tensor->src[1], tensor)) {
                return false;
            }
            // func = ggml_cuda_mul_mat_cont;
            func = ggml_cuda_mul_mat_cont_out;
            break;
        case GGML_OP_FREE_CUDA:
            if (!any_on_device && !ggml_cuda_can_mul_mat(tensor->src[0], tensor->src[1], tensor)) {
                return false;
            }
            func = ggml_cuda_free_tmp_data;
            break;
        case GGML_OP_CPY_W_CPU_GPU:
            if (!any_on_device) {
                return false;
            }
            func = ggml_cuda_cpy_w_cpu_gpu;
            break;
        case GGML_OP_RELU:
            if (!any_on_device) {
                return false;
            }
            func = ggml_cuda_op_relu;
            break;
        case GGML_OP_SCALE:
            if (!any_on_device) {
                return false;
        ...
        case GGML_OP_CPY:
            ...
            func = ggml_cuda_cpy;
            break;
        case GGML_OP_CPY_CPU_GPU:
            if (!any_on_device) {
                return false;
            }
            func = ggml_cuda_cpy_cpu_gpu;
            break;
        case GGML_OP_CONT:
            if (!any_on_device) {
                return false;
