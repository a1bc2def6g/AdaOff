...
static const char * GGML_OP_NAME[GGML_OP_COUNT] = {
    ...
    "CPY_CPU_GPU",
    "GGML_OP_MAT_MUL_CONT",
    "GGML_OP_FREE_CUDA",
    "GGML_OP_CPY_W_CPU_GPU"
};
...
struct ggml_tensor * ggml_cpy_cpu_gpu(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int32_t               il,
        int32_t               n_ctx,
    ...
    // const int32_t params[GGML_MAX_OP_PARAMS/sizeof(int32_t)] = {il,n_ctx,N,n_past,n_embd_gqa};

    int32_t params[] = {il, n_ctx, N, n_past, n_embd_gqa, cpu2gpu?1:0, is_v?1:0};

    struct ggml_tensor * result = ggml_new_tensor_impl(ctx, a->type, 1, a->ne, a->data);

    ggml_format_name(result, "%s (copy)", a->name);


    result->op   = GGML_OP_CPY_CPU_GPU;
    result->backend = GGML_BACKEND_GPU;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    // result->data = a->data;
    // result->type = a->type;
...
struct ggml_tensor * ggml_mat_mul_contiguous(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * b,
        int32_t               il,
    ...
    int32_t params[] = {il, n_ctx, N+n_past, N, n_embd_gqa, n_head, n_head_embd};

    const int64_t ne[4] = { b->ne[0], a->ne[1], a->ne[2], a->ne[3] };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, MAX(a->n_dims, b->n_dims), ne);

    // struct ggml_tensor * result = ggml_new_tensor_impl(ctx, a->type, 1, a->ne, NULL);

    ggml_format_name(result, "%s (mat_mul_cont)", a->name);


    result->op   = GGML_OP_MAT_MUL_CONT;
    result->backend = GGML_BACKEND_GPU;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    result->src[1] = b;
...
struct ggml_tensor * ggml_free_cuda(
        struct ggml_context * ctx,
        struct ggml_tensor  * a
        ) {
    ...
    struct ggml_tensor * result = ggml_new_tensor_impl(ctx, a->type, a->n_dims, a->ne, a->data);

    // struct ggml_tensor * result = ggml_new_tensor_impl(ctx, a->type, 1, a->ne, NULL);

    ggml_format_name(result, "%s (free_cuda)", a->name);


    result->op   = GGML_OP_FREE_CUDA;
    result->backend = GGML_BACKEND_GPU;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    result->extra = a->extra;
...
struct ggml_tensor * ggml_cpy_w_cpu_gpu(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    ...
    ggml_format_name(result, "%s (copy_w)", a->name);


    result->op   = GGML_OP_CPY_W_CPU_GPU;
    result->backend = GGML_BACKEND_GPU;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
    // result->data = a->data;
